...
% Changes based on type of launch
if fixedPitch
    theta = targetPitch;
elseif verticalLaunch
    ...
    theta = psi + Gimbal;   % This corrects cos(theta-psi) to be cos(gim)
end

% Change in mass
dmdt = -mdot;

% Change in velocity (depends on if fixed pitch, vertical launch, or other)
dvdt = -g0*sin(psi) + (Thrust/m)*cos(theta-psi) - (D/m);

% Change in position
drdt = v*sin(psi);

% Change in Flight Angle
dpsidt = -(g0/v - v/r)*cos(psi) + Thrust/(v*m)*sin(theta-psi);
